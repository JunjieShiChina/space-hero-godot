...
func _ready() -> void:
	...
	add_child(root)
	coin_label = _label(Vector2(24, 18), "COIN 0", 28)
	root.add_child(coin_label)
	weapon_label = _label(Vector2(24, 56), "WEAPON Bullet1", 22)
	root.add_child(weapon_label)
	health_bar = ProgressBar.new()
	health_bar.position = Vector2(24, 92)
	health_bar.size = Vector2(260, 22)
	health_bar.max_value = GameData.max_health
	root.add_child(health_bar)
	boss_bar = ProgressBar.new()
	boss_bar.position = Vector2(390, 22)
	boss_bar.size = Vector2(500, 24)
	boss_bar.max_value = 1.0
	boss_bar.visible = false
	root.add_child(boss_bar)
	warning_label = _label(Vector2(0, 280), "", 64)
	warning_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	warning_label.size = Vector2(1280, 90)
